...
try {
  ...
  if (isset($_GET['tag'])) {
    ...
    // 該当するタグの記事を取得
    $stmt = $pdo->prepare("SELECT article.article_id, title, content, insert_date, update_date, employee.employee_name, GROUP_CONCAT(tag.tag_name SEPARATOR ', ') AS tags
                           FROM article
    ...
    <ul class="item-wrapper">
      ...
      ?>
        <li>
          ...
          <button class="edit-btn btn"><a href="edit.php?id=<?php echo htmlspecialchars($article['article_id']); ?>">更新</a></button>
          <button class="delete-btn btn" data-article-id="<?php echo htmlspecialchars($article['article_id']); ?>">削除</button>
        </li>
      <?php } ?>
    </ul>
